<template>
  ...
  >
    ...
    <div :class="cpt_selection_fixed_left_class">
      <!-- current -->
      <div v-if="cpt_fixed_left_selection_current.selectionCurrent" v-html="cpt_fixed_left_selection_current.selectionCurrent"></div>
      <!-- area -->
      <div v-if="cpt_fixed_left_selection_area.normalArea" v-html="cpt_fixed_left_selection_area.normalArea"></div>
      <!-- auto fill -->
      <div v-if="cpt_fixed_left_auto_fill_area" v-html="cpt_fixed_left_auto_fill_area"></div>
      <!-- area layer -->
      <div v-if="cpt_fixed_left_selection_area.normalAreaLayer" v-html="cpt_fixed_left_selection_area.normalAreaLayer"></div>
    </div>
    
    <!-- middle -->
    <div :class="cpt_selection_middle_class">
      <!-- current -->
      <div v-if="cpt_middle_selection_current.selectionCurrent" v-html="cpt_middle_selection_current.selectionCurrent"></div>
      <!-- area -->
      <div v-if="cpt_middle_selection_area.normalArea" v-html="cpt_middle_selection_area.normalArea"></div>
      <!-- auto fill -->
      <div v-if="cpt_middle_auto_fill_area" v-html="cpt_middle_auto_fill_area"></div>
      <!-- area layer -->
      <div v-if="cpt_middle_selection_area.normalAreaLayer" v-html="cpt_middle_selection_area.normalAreaLayer"></div>
    </div>
    
    <!-- fixed right -->
    <div :class="cpt_selection_fixed_right_class">
      <!-- current -->
      <div v-if="cpt_fixed_right_selection_current.selectionCurrent" v-html="cpt_fixed_right_selection_current.selectionCurrent"></div>
      <!-- area -->
      <div v-if="cpt_fixed_right_selection_area.normalArea" v-html="cpt_fixed_right_selection_area.normalArea"></div>
      <!-- auto fill -->
      <div v-if="cpt_fixed_right_auto_fill_area" v-html="cpt_fixed_right_auto_fill_area"></div>
      <!-- area layer -->
      <div v-if="cpt_fixed_right_selection_area.normalAreaLayer" v-html="cpt_fixed_right_selection_area.normalAreaLayer"></div>
    </div>
  </div>
</template>
  ...
  return Vue.defineComponent({
    ...
    computed: {
      ...
      cpt_selection_borders_visibility() {
        ...
        if (this.isVirtualScroll) {
          ...
          if (showVirtualScrollingPlaceholder) {
            ...
            if (currentCellSelectionType === Vue._X_TABLE_EASY_CONSTANTS.CURRENT_CELL_SELECTION_TYPES.SINGLE) {
              if (
            ...
            if (currentCellSelectionType === Vue._X_TABLE_EASY_CONSTANTS.CURRENT_CELL_SELECTION_TYPES.RANGE) {
              if (
                (currentCell.rowIndex < virtualScrollVisibleIndexs.start &&
      ...
      cpt_show_corner() {
        ...
        if (cellAutofillOption) {
          ...
          if (Vue._X_TABLE_EASY_UTILS.isBoolean(directionY) && !directionY && Vue._X_TABLE_EASY_UTILS.isBoolean(directionX) && !directionX) {
            result = false;
          }
        } else {
          result = false;
      ...
      cpt_corner_cell_info() {
        ...
        if (Vue._X_TABLE_EASY_UTILS.isLastColumnByColKey(rightColKey, colgroups)) {
          ...
          if (
            colgroups[index + 1].fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT &&
            colgroups[index].fixed !== Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT
          ) {
            isLastColumn = true;
          }
        }
      ...
      cpt_is_first_not_fixed_selection_col() {
        ...
        if (colgroups.find(x => x.fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT)) {
      ...
      cpt_fixed_left_selection_current() {
        return this.cpt_get_selection_current({ fixedType: Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT });
      },
      // fixed left selection area
      cpt_fixed_left_selection_area() {
        return this.cpt_get_selection_areas({ fixedType: Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT });
      },
      ...
      cpt_fixed_left_auto_fill_area() {
        ...
        return cpt_fixed_left_selection_current.autoFillArea || cpt_fixed_left_selection_area.autoFillArea;
      },
      ...
      cpt_middle_auto_fill_area() {
        ...
        return cpt_middle_selection_current.autoFillArea || cpt_middle_selection_area.autoFillArea;
      },
      // fixed right selection current
      cpt_fixed_right_selection_current() {
        return this.cpt_get_selection_current({ fixedType: Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT });
      },
      // fixed right selection area
      cpt_fixed_right_selection_area() {
        return this.cpt_get_selection_areas({ fixedType: Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT });
      },
      ...
      cpt_fixed_right_auto_fill_area() {
        ...
        return cpt_fixed_right_selection_current.autoFillArea || cpt_fixed_right_selection_area.autoFillArea;
      }
    },
    ...
    watch: {
      parentRendered: {
        handler: function (val) {
          if (val) {
            ...
            this.hooks.addHook(Vue._X_TABLE_EASY_CONSTANTS.HOOKS_NAME.TABLE_CONTAINER_SCROLL, () => {
            ...
            this.hooks.addHook(Vue._X_TABLE_EASY_CONSTANTS.HOOKS_NAME.TABLE_SIZE_CHANGE, () => {
              // debounce reset cell positions
              this.debounceResetCellPositions();
            });
            // add table td width change hook
            this.hooks.addHook(Vue._X_TABLE_EASY_CONSTANTS.HOOKS_NAME.TABLE_CELL_WIDTH_CHANGE, () => {
              this.$nextTick(() => {
                this.cpt_reset_cell_positions();
              });
            });
            ...
            this.hooks.addHook(Vue._X_TABLE_EASY_CONSTANTS.HOOKS_NAME.CLIPBOARD_CELL_VALUE_CHANGE, () => {
              this.$nextTick(() => {
                this.cpt_reset_cell_positions();
              });
            });
      ...
      'cellSelectionData.currentCell': {
        handler: function (val) {
          ...
          if (!Vue._X_TABLE_EASY_UTILS.isEmptyValue(rowKey) && !Vue._X_TABLE_EASY_UTILS.isEmptyValue(colKey)) {
            this.cpt_set_current_cell_el();
            this.cpt_set_selection_positions({ type: 'currentCell' });
          } else {
            this[Vue._X_TABLE_EASY_CONSTANTS.INSTANCE_METHODS.CLEAR_CURRENT_CELL_RECT]();
          }
      ...
      'cellSelectionData.normalEndCell': {
        handler: function (val) {
          ...
          if (!Vue._X_TABLE_EASY_UTILS.isEmptyValue(rowKey) && !Vue._X_TABLE_EASY_UTILS.isEmptyValue(colKey)) {
            ...
            this[Vue._X_TABLE_EASY_CONSTANTS.INSTANCE_METHODS.CLEAR_NORMAL_END_CELL_RECT]();
          }
          this.cpt_set_cell_selection_range_data();
        },
        deep: true,
    ...
    methods: {
      ...
      cpt_reset_cell_positions() {
        ...
        if (!Vue._X_TABLE_EASY_UTILS.isEmptyValue(currentCell.rowKey) && !Vue._X_TABLE_EASY_UTILS.isEmptyValue(currentCell.colKey)) {
          this.cpt_set_selection_positions({
            type: 'currentCell'
          });
        }

        if (!Vue._X_TABLE_EASY_UTILS.isEmptyValue(normalEndCell.rowKey) && !Vue._X_TABLE_EASY_UTILS.isEmptyValue(normalEndCell.colKey)) {
          this.cpt_set_selection_positions({
            type: 'normalEndCell'
          });
        }
      ...
      cpt_set_cell_selection_range_data() {
        ...
        } else if (currentCellSelectionType === Vue._X_TABLE_EASY_CONSTANTS.CURRENT_CELL_SELECTION_TYPES.RANGE) {
        ...
        this.$emit(Vue._X_TABLE_EASY_CONSTANTS.EMIT_EVENTS.CELL_SELECTION_RANGE_DATA_CHANGE, result);
      },
      ...
      cpt_get_cell_position_by_col_key({ tableLeft, tableTop, colKey, isFirstRow, isLastRow }) {
        if (!this.cpt_selection_borders_visibility) {
          return false;
        }
      ...
      */
      ...
      cpt_get_selection_autofill_area({ areaPostions, fixedType }) {
        ...
        else if (rightmostColKey === autoFillEndCell.colKey && !Vue._X_TABLE_EASY_UTILS.isEmptyValue(rightmostColKey)) {
        ...
        else if (leftmostColKey === autoFillEndCell.colKey && !Vue._X_TABLE_EASY_UTILS.isEmptyValue(leftmostColKey)) {
      ...
      cpt_get_borders({
        ...
        const cornerProps = {
          ...
          on: {
            mousedown: e => {
              this.dispatch(Vue._X_TABLE_EASY_COMPS_NAME.VE_TABLE, Vue._X_TABLE_EASY_CONSTANTS.EMIT_EVENTS.SELECTION_CORNER_MOUSEDOWN, {
                event: e
              });
            },
            mouseup: e => {
              this.dispatch(Vue._X_TABLE_EASY_COMPS_NAME.VE_TABLE, Vue._X_TABLE_EASY_CONSTANTS.EMIT_EVENTS.SELECTION_CORNER_MOUSEUP, {
                event: e
              });
            }
          }
        ...
        // corner
        html += `<div class="${cornerProps.class}" style="${Object.entries(cornerProps.style).map(([key, value]) => `${key}: ${value}`).join('; ')}" onmousedown="${cornerProps.on.mousedown.toString()}" onmouseup="${cornerProps.on.mouseup.toString()}"></div>`;
