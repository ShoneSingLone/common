    ...
    computed: {
      ...
      cpt_is_last_left_fixed_column() {
        ...
        if (fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT) {
          const { field } = column;
          const leftFixedColumns = this.$parent.colgroups.filter(x => x.fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT);
      ...
      cpt_is_first_right_fixed_column() {
        ...
        if (fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT) {
          const { field } = column;
          const rightFixedColumns = this.$parent.colgroups.filter(x => x.fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT);
      ...
      cpt_footer_td_class() {
        ...
        // column fixed
        if (fixed) {
          result[Vue._X_TABLE_EASY_UTILS.clsName('fixed-left')] = fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT;
          result[Vue._X_TABLE_EASY_UTILS.clsName('fixed-right')] = fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT;
          result[Vue._X_TABLE_EASY_UTILS.clsName('last-left-fixed-column')] = this.cpt_is_last_left_fixed_column;
          result[Vue._X_TABLE_EASY_UTILS.clsName('first-right-fixed-column')] = this.cpt_is_first_right_fixed_column;
        }
      ...
      cpt_footer_td_style() {
        ...
        if (fixed) {
          ...
          if (
            (fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT && columnIndex > 0) ||
            (fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT && columnIndex < $parent.colgroups.length - 1)
          ) {
          ...
          result['left'] = fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT ? totalWidth : '';
          result['right'] = fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT ? totalWidth : '';
        }
