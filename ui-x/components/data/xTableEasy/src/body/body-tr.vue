  ...
  return Vue.defineComponent({
    ...
    methods: {
      ...
      rowClick(e, fn) {
        ...
        this.dispatch(Vue._X_TABLE_EASY_COMPS_NAME.VE_TABLE_BODY, Vue._X_TABLE_EASY_EMIT_EVENTS.BODY_ROW_CLICK, {
          rowData,
          rowIndex
        });
      },
    ...
    render(h) {
      ...
      const getTdContent = () => {
        return colgroups.map(column => {
          const tdProps = {
            ...
            on: {
              [Vue._X_TABLE_EASY_EMIT_EVENTS.EXPAND_ROW_CHANGE]: () => expandRowChange(rowData, rowIndex)
            }
          };
      ...
      if (this.isVirtualScroll) {
        const props = {
          ...
          attrs: {
            [Vue._X_TABLE_EASY_CONSTANTS.COMPS_CUSTOM_ATTRS.BODY_ROW_KEY]: this.cpt_current_row_key
          },
          on: {
            "on-dom-resize-change": ({ key, height }) => {
              this.dispatch(Vue._X_TABLE_EASY_COMPS_NAME.VE_TABLE, Vue._X_TABLE_EASY_EMIT_EVENTS.BODY_ROW_HEIGHT_CHANGE, {
                rowKey: key,
                height
              });
            }
        ...
        result = h(Vue._X_TABLE_EASY_COMPONENTS.VueDomResizeObserver, props, getTdContent());
      } else {
        const props = {
          class: this.cpt_tr_class,
          attrs: {
