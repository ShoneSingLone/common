        ...
        <div :class="cpt_table_content_wrapper_class" :ref="tableContentWrapperRef">
          ...
          <table :class="cpt_table_class" :ref="tableRef" :style="cpt_table_style">
            ...
            <body-component 
              ...
              @cell-selection-normal-end-cell-change="cellSelectionNormalEndCellChange"
              ...
              @is-body-operation-column-mousedown-change="isBodyOperationColumnMousedownChange"
          ...
          <div 
            ...
            :style="cpt_virtual_phantom_style"
          ></div>
        </div>
      </div>
    </div>
  ...
  return Vue.defineComponent({
    ...
    computed: {
      ...
      cpt_table_container_class() {
        ...
        return {
          ...
          [Vue._X_TABLE_EASY_UTILS.clsName("container-right-scrolling")]: isRightScrolling,
          [Vue._X_TABLE_EASY_UTILS.clsName("container-vertical-scrolling")]: isVerticalScrolling,
          ...
          [Vue._X_TABLE_EASY_UTILS.clsName("enable-cell-selection")]: cpt_enable_cell_selection
        };
      },
      ...
      cpt_table_container_style() {
        ...
        if (this.cpt_is_virtual_scroll) {
          ...
          tableContainerHeight = Vue._X_TABLE_EASY_UTILS.getValByUnit(tableContainerHeight);
        }
      ...
      cpt_has_fixed_column() {
        return this.colgroups.some(
          x => x.fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT || x.fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT
        );
      },
      // has left fixed column
      cpt_has_left_fixed_column() {
        return this.colgroups.some(x => x.fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.LEFT);
      },
      // has right fixed column
      cpt_has_right_fixed_column() {
        return this.colgroups.some(x => x.fixed === Vue._X_TABLE_EASY_CONSTANTS.COLUMN_FIXED_TYPE.RIGHT);
      },
      // is editing cell
      cpt_is_cell_editing() {
        const { editingCell } = this;

        return !Vue._X_TABLE_EASY_UTILS.isEmptyValue(editingCell.rowKey) && !Vue._X_TABLE_EASY_UTILS.isEmptyValue(editingCell.colKey);
      },
      // has edit column
      cpt_has_edit_column() {
    ...
    methods: {
      ...
      bodyCellWidthChange(colWidths) {
        ...
        this.hooks.triggerHook(Vue._X_TABLE_EASY_CONSTANTS.HOOKS_NAME.TABLE_CELL_WIDTH_CHANGE);
      },
      ...
      showOrHideColumns() {
        ...
        if (!Vue._X_TABLE_EASY_UTILS.isEmptyArray(hiddenColumns)) {
          ...
          hiddenColumns.forEach(key => {
            cloneColumns = Vue._X_TABLE_EASY_UTILS.recursiveRemoveColumnByKey(cloneColumns, key);
          });
        }
      ...
      selectedAllChange({ isSelected }) {
        this.broadcast(Vue._X_TABLE_EASY_COMPS_NAME.VE_TABLE_BODY, Vue._X_TABLE_EASY_CONSTANTS.EMIT_EVENTS.CHECKBOX_SELECTED_ALL_CHANGE, {
          isSelected
        });
      },
      ...
      setCellSelectionByAutofill() {
        ...
        if (currentCellSelectionType === Vue._X_TABLE_EASY_CONSTANTS.CURRENT_CELL_SELECTION_TYPES.RANGE) {
          if (
            ...
            if (autofillingDirection === Vue._X_TABLE_EASY_CONSTANTS.AUTOFILLING_DIRECTION.RIGHT) {
              currentCellData = {
                rowKey: topRowKey,
                colKey: leftColKey
              };
              normalEndCellData = { rowKey: bottomRowKey, colKey };
            } else if (autofillingDirection === Vue._X_TABLE_EASY_CONSTANTS.AUTOFILLING_DIRECTION.DOWN) {
              currentCellData = {
                rowKey: topRowKey,
                colKey: leftColKey
              };
              normalEndCellData = { rowKey, colKey: rightColKey };
            } else if (autofillingDirection === Vue._X_TABLE_EASY_CONSTANTS.AUTOFILLING_DIRECTION.UP) {
              currentCellData = {
                rowKey,
                colKey: leftColKey
              };
            ...
            } else if (autofillingDirection === Vue._X_TABLE_EASY_CONSTANTS.AUTOFILLING_DIRECTION.LEFT) {
        ...
        else if (currentCellSelectionType === Vue._X_TABLE_EASY_CONSTANTS.CURRENT_CELL_SELECTION_TYPES.SINGLE) {
      ...
      dealKeydownEvent(event) {
        const { colgroups, cellSelectionData, enableStopEditing, cpt_is_cell_editing } = this;
        ...
        if (!Vue._X_TABLE_EASY_UTILS.isEmptyValue(rowKey) && !Vue._X_TABLE_EASY_UTILS.isEmptyValue(colKey)) {
          switch (keyCode) {
            case Vue._X_TABLE_EASY_CONSTANTS.KEY_CODES.TAB: {
              let direction;
              if (shiftKey) {
                direction = Vue._X_TABLE_EASY_CONSTANTS.CELL_SELECTION_DIRECTION.LEFT;
              } else {
            ...
            case Vue._X_TABLE_EASY_CONSTANTS.KEY_CODES.ARROW_LEFT: {
              const direction = Vue._X_TABLE_EASY_CONSTANTS.CELL_SELECTION_DIRECTION.LEFT;
            ...
            case Vue._X_TABLE_EASY_CONSTANTS.KEY_CODES.ARROW_RIGHT: {
              const direction = Vue._X_TABLE_EASY_CONSTANTS.CELL_SELECTION_DIRECTION.RIGHT;
            ...
            case Vue._X_TABLE_EASY_CONSTANTS.KEY_CODES.ARROW_UP: {
              const direction = Vue._X_TABLE_EASY_CONSTANTS.CELL_SELECTION_DIRECTION.UP;
            ...
            case Vue._X_TABLE_EASY_CONSTANTS.KEY_CODES.ARROW_DOWN: {
              const direction = Vue._X_TABLE_EASY_CONSTANTS.CELL_SELECTION_DIRECTION.DOWN;
      ...
      selectCellByDirection({ direction }) {
        ...
        if (direction === Vue._X_TABLE_EASY_CONSTANTS.CELL_SELECTION_DIRECTION.LEFT) {
        ...
        } else if (direction === Vue._X_TABLE_EASY_CONSTANTS.CELL_SELECTION_DIRECTION.RIGHT) {
          ...
          if (rowIndex > 0) {
            ...
            this.rowToVisible(Vue._X_TABLE_EASY_CONSTANTS.KEY_CODES.ARROW_UP, nextRowKey);
          }
        } else if (direction === Vue._X_TABLE_EASY_CONSTANTS.CELL_SELECTION_DIRECTION.DOWN) {
          if (rowIndex < allRowKeys.length - 1) {
            const nextRowKey = allRowKeys[rowIndex + 1];
            this.rowToVisible(Vue._X_TABLE_EASY_CONSTANTS.KEY_CODES.ARROW_DOWN, nextRowKey);
          }
        }
      },
      ...
      columnToVisible(nextColumn) {
        ...
        if (!nextColumn.fixed) {
          ...
          const rightTotalWidth = Vue._X_TABLE_EASY_UTILS.getNotFixedTotalWidthByColumnKey({
            colgroups,
            colKey: nextColumn.key,
      ...
      rowToVisible(keyCode, nextRowKey) {
        ...
        const { cpt_is_virtual_scroll, cpt_header_total_height, cpt_footer_total_height } = this;
        ...
        if (nextRowEl) {
          ...
          // arrow down
            ...
            if (trOffsetTop + trClientHeight > containerScrollTop + containerClientHeight) {
              tableContainerRef.scrollTop = trOffsetTop + trClientHeight - containerClientHeight + parentOffsetTop;
            }
          }
        }
      },
      ...
      calculateTableHeight() {
        if (!this.cpt_is_virtual_scroll) {
          ...
          if (tableBodyRef) {
            ...
            this.tableHeight = this.cpt_header_total_height + offsetHeight + this.cpt_footer_total_height;
          }
        }
      },
      ...
      clearVirtualScrollTimer() {
        if (this.disablePointerEventsTimeoutId) {
          Vue._X_TABLE_EASY_UTILS.cancelAnimationTimeout(this.disablePointerEventsTimeoutId);
