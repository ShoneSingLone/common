...
`routes.vue` 里目前存在不少 **同一路径重复声明** 的情况，例如同一个 `x-table-easy` 功能路径，既指向 `xTableEasy.*.vue`，又指向中文命名的旧示例页。
...
如果你认可这个理解，我下一轮可以继续帮你把这部分再整理进 `xTableEasy-迁移实施计划.md`，让后面的“移植组件 + 同步文档 + 同步测试”形成一套统一执行规则。
...
2. 已新增脚本 `scripts/check-x-table-easy-imports.js`，并完成一轮扫描：
   ...
   - 缺失导入数：0
3. 当前仍需重点验证的风险：
   - `header-filter-custom-content.vue` 没有像源实现那样显式导入 `VeDropdown`、`VeIcon`，而是依赖字符串组件名，需要页面验证运行时是否可靠
   - `business_doc/router/routes.vue` 里同一路径重复声明，新旧页面入口并存，后面需要统一正式入口
4. 新补的 `.vue` 包装文件在编辑器里出现了 ts-plugin “multiple default exports” 提示，但从现有 `util/constant.vue` 同类写法和扫描结果看，当前更像 IDE/插件误报，暂时不作为阻塞结论
